...
Requires `node` and `jq`, both already in the repo's `Brewfile`. No npm
...
the copy, and a later `git pull` in the repo will not reach them. If the
repo is meant to be the source of truth, run `install.sh` from the repo
checkout, or install the skill as a symlink rather than with `--copy`.
...
The key never belongs in this repo. Resolve it at launch rather than in
every shell:
...
without editing the repo. `lib/config.mjs` is the full list with current
