...
command -v jq   >/dev/null || { warn "jq is required (it is in the Brewfile)"; exit 1; }
command -v node >/dev/null || { warn "node is required (it is in the Brewfile)"; exit 1; }

# Strips any hook entry pointing at this repo, at any nesting level, so the
# script is safe to re-run and safe to run after the repo has moved.
...
case "${1:-install}" in
  --check)
    say "Hooks currently registered from this repo"
    jq --arg d "$JEV_DIR" '
      [ .hooks // {} | to_entries[]
        | .key as $event
